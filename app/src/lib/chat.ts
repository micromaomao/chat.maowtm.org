...
import { FetchedChatMessage, FetchedChatMessageMetadata, msgTypeToStr } from "../api/v1/types"
...
export async function fetchChatSession(session_id: string, db_client: DBClient): Promise<object | null> {
  let { rows } = await db_client.query({
